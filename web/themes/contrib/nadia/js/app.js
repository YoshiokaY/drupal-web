...
/***/ "./src/_assets/js/utils/getJson.js":
/*!*****************************************!*\
  !*** ./src/_assets/js/utils/getJson.js ***!
  \*****************************************/
...
//jsonを指定
var archive = "http://mydrupal.lndo.site/jsonapi/node/news/";
//表示箇所
var dl = document.querySelector(".newsList");
function getJson(_x) {
...
// }
function _getJson() {
  _getJson = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(url) {
    var response, json;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        ...
        case 11:
          ...
          //200以外が返ってきた場合
          console.log("JSONを取得できませんでした：" + _context.t0);
          setHtml(null, dl);
          // return null;
        case 15:
        case "end":
...
function setHtml(dataList, el) {
  var html = "";
  if (dataList === null) {
    html += "<p>記事取得中にエラーが発生しました。<br>時間をおいてから再度アクセスしてください。</p>";
  } else if (el) {
    dataList.forEach(function (element) {
      var time = new Date(element.attributes.created);
      var cat_id = element.relationships.field_katekori.data.id;
      var cat = cat_id == "24a2aad5-8ae3-4251-aac4-b27d35f908b9" ? ["お知らせ", "info"] : cat_id == "d27d631b-65ef-4e52-8b67-7d9326898ddc" ? ["ニュースリリース", "release"] : ["イベント", "event"];
      html += '<div class="newsList_item">';
      html += "<dt>";
      html += "<time>".concat(time.toLocaleDateString(), "</time>");
      html += "<span class=\"catBtn -".concat(cat[1], "\" data-cat=\"").concat(cat_id, "\">").concat(cat[0], "</span>");
      html += "</dt>";
      html += "<dd><a href=\"/news/?id=".concat(element.id, "\">").concat(element.attributes.title, "</a></dd>");
      html += "</div>";
    });
  }
  // HTMLに出力
...
var observer = new MutationObserver(function (mutations) {
  mutations.forEach(function (mutation) {
    if (mutation.addedNodes.length > 0) {
      var addElements = mutation.addedNodes;
      addElements.forEach(function (addElement) {
        var btn = addElement.querySelector(".catBtn");
        btn.addEventListener("click", function () {
          var target = btn.getAttribute("data-cat");
          console.log(target);
          var url = "http://mydrupal.lndo.site/jsonapi/node/news?filter[field_katekori]=" + target;
          // getJson(url);
        });
      });
    }
...
var target = document.querySelector(".newsList");
if (target) {
...
var acs = document.querySelectorAll(".sortBtn");
acs.forEach(function (btn) {
  btn.addEventListener("click", function () {
    var set = btn.getAttribute("data-sort");
    var desc = btn.getAttribute("data-desc");
    var url = "http://mydrupal.lndo.site/jsonapi/node/news/?sort[sort-".concat(set, "][path]=").concat(set);
    if (desc != "true") {
      getJson(url);
    } else {
      getJson(url + "&sort[sort-".concat(set, "][direction]=DESC"));
    }
  });
});
var filters = document.querySelectorAll(".filterBtn");
var filterurl = "http://mydrupal.lndo.site/jsonapi/node/news?filter[label-a][condition][path]=title&filter[label-a][condition][operator]=CONTAINS&filter[label-a][condition][value]=";
filters.forEach(function (filter) {
  filter.addEventListener("click", function () {
    var set = filter.textContent;
    var url = filterurl + set;
    console.log(url);
    getJson(url);
  });
});
var query = window.location.search.slice(4);
var single = archive + query;
var news = document.getElementById("news_body");
console.log(single);
function getNews(_x2) {
  return _getNews.apply(this, arguments);
}
function _getNews() {
  _getNews = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(url) {
    var response, json;
    return _regeneratorRuntime().wrap(function _callee2$(_context2) {
      while (1) switch (_context2.prev = _context2.next) {
        ...
        case 11:
          ...
          //200以外が返ってきた場合
          console.log("JSONを取得できませんでした：" + _context2.t0);
          // setNews(null, dl);
          // return null;
        case 14:
        case "end":
...
function setNews(dataList, el) {
  ...
  if (dataList === null) {
    ...
    var time = new Date(dataList.attributes.created);
    var cat_id = dataList.relationships.field_katekori.data.id;
    var cat = cat_id == "24a2aad5-8ae3-4251-aac4-b27d35f908b9" ? ["お知らせ", "info"] : cat_id == "d27d631b-65ef-4e52-8b67-7d9326898ddc" ? ["ニュースリリース", "release"] : ["イベント", "event"];
    var body = decodeURI(dataList.attributes.body.value).replace("/sites/", "http://mydrupal.lndo.site/sites/");
    html += "<h1 class=\"c_ttl_h1\">".concat(dataList.attributes.title, "</h1>");
    html += '<section><div class="contentInner">';
    html += "<ul><li><time>".concat(time.toLocaleDateString(), "</time></li><li><span class=\"catBtn -").concat(cat[1], "\">").concat(cat[0], "</span></li></ul>");
    html += "<article>".concat(body, "</article>");
    html += "</div></section>";
  }
  // HTMLに出力
  if (el) {
...
/*!*********************************!*\
  ...
  \*********************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _utils_getJson__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./utils/getJson */ "./src/_assets/js/utils/getJson.js");
// import "lity";
// import 'drawsvg';
