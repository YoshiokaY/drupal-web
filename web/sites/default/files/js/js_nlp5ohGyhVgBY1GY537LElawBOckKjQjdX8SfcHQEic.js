...
// let archive = "https://nadia.bz/wp-json/wp/v2/posts?per_page=2";
var archive2 = "http://mydrupal.lndo.site/jsonapi/node/voice/";
...
function setHtml(dataList, el) {
  var tr = "";
  if (dataList === null) {
    tr += "<p>記事取得中にエラーが発生しました。<br>時間をおいてから再度アクセスしてください。</p>";
  } else {
    dataList.forEach(function (element) {
      var time = new Date(element.attributes.created);
      tr += "<div>";
      tr += "<dt>";
      tr += "<time>".concat(time.toLocaleDateString(), "</time>");
      tr += "</dt>";
      tr += "<dd>".concat(element.attributes.title, "</dd>");
      tr += "</div>";
    });
  }
  // HTMLに出力
